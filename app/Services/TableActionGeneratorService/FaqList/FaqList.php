...
namespace App\Services\TableActionGeneratorService\FaqList;
use App\Models\Faq;
use App\Services\TableActionGeneratorService\Generator;
use App\Services\TableActionGeneratorService\GeneratorService;

class FaqList extends GeneratorService implements Generator
{
    private static ?FaqList $instance = null;

    protected Faq $faq;
    public function __construct(){}

    public static function getInstance(mixed $faq): Generator
    {
        if(self::$instance == null)
            self::$instance = new self();
        
        self::$instance->set($faq);
    ...
    {
        $editTo = route("addEditFaqPage", $this->faq->uid);
        $deleteTo = route("faqDelete", $this->faq->uid);
