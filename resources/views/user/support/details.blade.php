...
@section('content')
    ...
    <div class="container mx-auto p-6 space-y-6 bg-base-200 rounded-lg shadow-md">
            ...
            @if (filled($replies))
                @foreach ($replies as $replay)
                    <div class="flex items-start space-x-4 border-success border-b-2 ">
                        <img src="{{ $replay->sender_user?->getImage() }}" alt="Admin Image" class="w-10 h-10 rounded-full" loading="lazy" />
        ...
        @if ($ticket->status !== SupportTicketStatus::CLOSED)
            <!-- Reply Form -->
            <form action="{{ route('replaySupportTicket') }}" method="post" enctype="multipart/form-data">@csrf
    ...
    <script>
        function closeTicket() {
            Notiflix.Confirm.show(
                ...
                '{{ __('No') }}',
                () => {
                    window.location.href = '{{ route('closeTicketPage', $ticket->ticket) }}'
                }
            );
        }
